...
## Repo status
**Foundation COMPLETE (F0–F4) · P5 done · F3-UI redesigned · P6 done; P7–P9 are next (purely additive).** A Rust cargo workspace under `crates/` built phase by phase per `docs/TODO.md`. Current: **30 crates, 1002 workspace tests + 70 app (Vitest) tests green**, clippy `-D warnings` + rustfmt clean, tsc + vite build + Tauri `cargo check` green, no stub macros; on branch `feat/f3-ui-redesign` (PR #1). P6 added `spork-transport` (the `Transport` seam + subprocess + dep-free plaintext-HTTP impls) and `spork-agent` (the end-to-end agent-turn loop), the `node.agentRun` read-only run that attaches a priced `agent-context` node by a dotted edge (§6.6), the projection cost write-path, and the UI activation (model selector + cost ledger) — all behind frozen seams. Every load-bearing contract is frozen with exactly one real impl, so P7–P9 add implementations behind existing seams — never edit a frozen contract (see plan §9 D-1…D-15 first). `docs/TODO.md` is the live source of progress — read it for the exact next step.
