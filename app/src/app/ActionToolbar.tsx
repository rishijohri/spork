...
export function ActionToolbar({ node, ariaLabel }: ActionToolbarProps): JSX.Element {
  ...
  const onAction = useCallback(
    async (action: ToolbarAction): Promise<void> => {
      ...
      try {
        await runToolbarAction(action, node, { defaultModel }, { beginOptimistic });
      } finally {
        setInFlight((s) => {
    ...
    [node, defaultModel, beginOptimistic],
  );
