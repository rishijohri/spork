...
> **Build split (added during implementation).** F3 is delivered in two slices. The **F3 headless core** — typed IPC + `opId`/event reconciliation, dual-channel stream, capability broker, `vaultRef` vault, three-source drift capture, atomic dual-restore, git import/export — is built and verified driven by a **headless client over the real IPC** (no GUI needed). The **Tauri/React DAG canvas is split out as `F3-UI`**, a deferred slice (§11.5), because a desktop/webview UI cannot be built or visually verified in a headless CLI environment. All IPC/event/capability/`AttributionRecord`/view-model contracts are frozen in the F3 core, so `F3-UI` binds to frozen seams and is purely additive — **no rework** (C2). **F3-UI is scheduled immediately after P5** (order: …F4 → P5 → F3-UI → P6…; §11.5) — code-unblocked as of F4 with no hard P5 dependency, so its only remaining prerequisite is a GUI-capable environment (it can be pulled in earlier the moment one exists).
...
**Auto-branch on agent runs (DESIGN.md §6.6).** The agent-run path (click a node → ask analyze / plan / change) applies the daemon-owned **fork-on-divergence** policy: a code-changing turn from a branch *tip* continues it, from a *non-tip* node auto-forks a new branch; read-only analyze/plan attach as observing/context nodes (no fork). Additive over the caller-supplied `branch_id` on the frozen `create_node` seam — a new policy, not a contract change (§9).
...
**Auto-branch on historical checkout & drift (DESIGN.md §6.6).** "Check out this node" (materialize a non-tip node's worktree via the `F4` `WorktreeCowBackend`, behind a new read/checkout command) and the drift-reconcile path apply the same **fork-on-divergence** policy, so out-of-band human edits to a non-tip node land on an auto-forked branch rather than overwriting the line. Additive policy over the existing `create_node`/drift seams.
...
**Goal.** Make Spork usable as a **minimum viable product** *before* the additive P8/P9 features. The contract-first spine (F0–P7) froze every load-bearing seam and proved them headless, but **no phase ever owned the end-to-end user journey** — so by P7 a user who opens a real repo gets a permanently empty canvas (no IPC command, no UI affordance, and no daemon driver mints the first node). P7.5 closes that with **wiring-only workstreams over the shipped F4 seams** — additive `Command`/`AgentRunIntent` variants, new Tauri commands, new daemon modules, and the matching UI — **editing no frozen contract**. It operationalizes **DESIGN.md §17 "Phase 1 — Local MVP (the wedge)"**, which the spine had shattered across F/P phases.

**The five workstreams (all wiring-gaps — backend exists, expose it):** W1 onboarding/import (capture the working tree → first root snapshot node), W2 root the daemon at the user's real project dir, W3 provider/agent configuration from the app (point at `copilot-cli`/a local endpoint), W4 the **trusted-local code-mutating edit loop** (CoW worktree → tool loop → new Edit snapshot → §6.6 fork-on-divergence → auto-Sanity, on the host-kernel `WorktreeCow` tier only — **not** P8 untrusted isolation), W5 the conversation/transcript surface.

**Explicitly deferred (genuinely new infra, NOT MVP-blocking):** untrusted-executor isolation (the real P8), cloud TLS transport, and live token streaming.

**Definition of Done (in user terms):** a single user opens a real repo → sees their code as a root node → configures a local/CLI agent → asks the agent to make a change → gets a new Edit node (diff + transcript + auto-Sanity) that auto-forks on divergence → branches/merges/restores it → reopens the project intact.

**Full detail — backend AND UI, task by task, with design cross-references and a test plan — lives in [MVP_PLAN.md](MVP_PLAN.md).** This entry is the pointer; that file is authoritative for P7.5.

### Realignment (R0–R6) — Timeline-First · Node-Centric · Platform-Inverted

**Goal.** After the MVP shipped, running it surfaced that the UI had drifted *git-ward* (a branch sidebar/switcher, merge/push as primary verbs) and buried the spine — the **timeline of typed nodes** — and that the agent connectivity was broken (the generic-CLI provider impersonates a coding *agent* as a one-shot *model* over a protocol nothing speaks). The realignment re-centers Spork on its thesis: **nodes are the spine, branches are emergent lines, your existing agents drive Spork over MCP, editing hands off to your IDE.** It is **additive over every frozen F0–F4/P5–P7 contract** (a §7 C2 risk register, adversarially audited — e.g. per-type states ride as a `presentation_status` in the node payload, never touching the frozen `Lifecycle`; git-push/commit become nodes via a *new* node-producing command, never by editing the frozen event-less `GIT_PUSH`).

**The seven phases:** R0 docs (done) · R1 connectivity unblock · R2 view fields + UI de-git-ification + hero chat + open-in-editor · R3 agentic/action node-type descriptors + dispatch + state producer · R4 git-push/commit as nodes · R5 the write-capable **Orchestration MCP** (the "Spork Node skill") · R6 cloud BYOK TLS transport.

**Full detail — the connectivity decision, the additive node-type/state spec, the Orchestration MCP, the timeline-first UI reframe, the phased plan, and the C2 risk register — lives in [REALIGNMENT_PLAN.md](REALIGNMENT_PLAN.md).** This entry is the pointer; that file is authoritative for the realignment. **R0 (docs) done; R1–R6 proposed, not built.**

**dependsOn.** P5 (built-in node types), F4 (execution/provider/context seams). Sequenced **before P8** (an MVP should precede additive features). Purely additive — does not block or reorder P6/P7, which are already complete.
...
| **P7.5 — Local Trusted-Agent MVP** *(MVP Readiness — see [MVP_PLAN.md](MVP_PLAN.md))* | Wiring-only over shipped F4 seams: onboard/import a repo → first root node; daemon roots at the real project dir; provider config from the app (copilot-cli/local); trusted-local code-mutating edit loop (CoW worktree → Edit snapshot → §6.6 fork → auto-Sanity); conversation/transcript surface. | No new contract — additive `Command`/`AgentRunIntent` variants + Tauri commands + UI. Operationalizes DESIGN §17. | A user opens a real repo, sees their code as a root node, configures a local/CLI agent, asks it to make a change, and gets an Edit node (diff + transcript + auto-Sanity) that auto-forks on divergence — then branches/merges/restores it and reopens intact. | Untrusted isolation → P8; cloud TLS + token streaming → additive post-MVP |
...
Five capabilities are deliberately deferred (the first four match DESIGN.md §17 Phase 4 and §9.3; the fifth, F3-UI, was split out during implementation). Deferral is safe precisely because each one's load-bearing seam is **frozen in an earlier phase with one working implementation behind it** — so the deferred item arrives as a new implementation behind an existing interface (C3), never a core edit (C2) and never a stub backfill (C1).
...
### 11.5 F3-UI — the Tauri/React DAG canvas (added during implementation)

**Scheduled position (concrete):** the implementation order is `F0→F1→F2→F3-core→F4 (complete) → P5 → **F3-UI** → P6 → P7 → P8 → P9`. F3-UI is the phase **immediately after P5**, so the first canvas renders the real Edit/Validation/Stress/Sanity/Merge node types. **It is code-unblocked as of F4** — it has NO hard dependency on P5 (it is schema-driven off `NodeTypeDescriptor` and binds only to frozen F3 contracts), so it MAY be pulled in earlier (even before P5) the instant a GUI environment exists. **The single remaining prerequisite is environmental, not sequential:** a **GUI-capable dev environment** (display + Tauri/Node toolchain + a human for visual/interaction verification). It cannot be built or verified in a headless CLI sandbox — it would not meet the green-and-proven bar every other phase clears — so it is not attempted there; P6–P9 are backend and proceed independently of it. **Why no cascade:** the F3 headless core freezes every contract the renderer binds to — the typed IPC command/event envelope, the `opId`-returns-then-state-via-events reconciliation rule, the durable-vs-ephemeral channel split, the capability vocabulary, the `AttributionRecord` schema, and the **view-model boundary** (D-8; §14.3 isolates layout authority so Q8 stays deferrable). The renderer is a pure consumer of that frozen surface, exercised meanwhile by a headless client, so F3-UI is a new front-end behind unchanged seams — additive, never a rewrite of the daemon. Node cards/details/legend are already schema-driven off the `NodeTypeDescriptor` (D-6), so they render built-in and custom types identically with no core edit.

**Cross-cutting safety proof.** For all five deferrals the same invariant holds: the seam (interface, versioned schema, or registry) exists from the foundation with ≥1 production-grade implementation, every persisted artifact carries a `schema_version` with a forward-migration path (C5), and the deferred work is additive-only behind that seam (C2/C3). No deferred item leaves a stub, a TODO, or a hollow shell in any earlier phase (C1).
