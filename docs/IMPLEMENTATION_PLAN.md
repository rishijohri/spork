...
> **Build split (added during implementation).** F3 is delivered in two slices. The **F3 headless core** — typed IPC + `opId`/event reconciliation, dual-channel stream, capability broker, `vaultRef` vault, three-source drift capture, atomic dual-restore, git import/export — is built and verified driven by a **headless client over the real IPC** (no GUI needed). The **Tauri/React DAG canvas is split out as `F3-UI`**, a deferred slice (§11.5), because a desktop/webview UI cannot be built or visually verified in a headless CLI environment. All IPC/event/capability/`AttributionRecord`/view-model contracts are frozen in the F3 core, so `F3-UI` binds to frozen seams and is purely additive — **no rework** (C2). **F3-UI pick-up trigger:** a GUI-capable dev environment is available AND the headless backend is complete through at least F4; recommended slot is right after P5 (so the first canvas renders real typed nodes).
...
**Deferred to:** when a **GUI-capable dev environment** is available (display + Tauri toolchain + a human for visual/interaction verification) **and** the headless backend is complete through at least **F4**; **recommended slot: right after P5** (so the first canvas renders real Edit/Validation/Stress/Sanity/Merge nodes). Deferred because a desktop/webview UI cannot be built or visually verified in a headless CLI sandbox — it would not meet the green-and-proven bar every other phase clears. **Why no cascade:** the F3 headless core freezes every contract the renderer binds to — the typed IPC command/event envelope, the `opId`-returns-then-state-via-events reconciliation rule, the durable-vs-ephemeral channel split, the capability vocabulary, the `AttributionRecord` schema, and the **view-model boundary** (D-8; §14.3 isolates layout authority so Q8 stays deferrable). The renderer is a pure consumer of that frozen surface, exercised meanwhile by a headless client, so F3-UI is a new front-end behind unchanged seams — additive, never a rewrite of the daemon. Node cards/details/legend are already schema-driven off the `NodeTypeDescriptor` (D-6), so they render built-in and custom types identically with no core edit.
