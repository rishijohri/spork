...
> **MVP-readiness surfaces (P7.5 — added 2026-06-21; Slices A + B landed 2026-06-21).** These surfaces are what made Spork a usable **minimum viable product**, and they are now **🟢 BUILT**: the **onboarding / project import** affordance (the canvas empty-state runs the real `open_project → PROJECT_IMPORT` flow that captures a real repo into the first root node — the empty-canvas dead-end is gone); **provider/agent configuration** in Settings (the Provider section points at a local OpenAI-compatible endpoint — the default route — or a *conforming* CLI agent, persisted to `<project>/.spork/agent_config.json`; the top-bar model selector is gated to backed providers via `availableModels`; the generic CLI-as-model route is **deprecated** — see the realignment note below); the **"Make a change" (code-mutating) agent run** (the Ask modal's `change` intent dispatches `NODE_AGENT_EDIT` → an Edit node owning the CoW-mutated snapshot, auto-forking on divergence); and the **Conversation tab** in Node-Details (the node's stored transcript via `get_node_transcript` + a threaded read-only follow-up). These are consolidated under **[IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) §P7.5** and specified task-by-task (UI **and** backend) in **[MVP_PLAN.md](MVP_PLAN.md)**. **Still 🟡/deferred (post-MVP / P8):** live token **streaming** over `CHAT_TOKENS` (the RunRail stdout tab stays disabled until a streaming producer exists), cloud-**TLS** providers, and untrusted executors.

> **Realignment surfaces (R0 — added 2026-06; 🟡 designed, not yet built — see [REALIGNMENT_PLAN.md](REALIGNMENT_PLAN.md) §5).** The MVP UI drifted git-ward; the realignment makes the **node timeline the hero** and de-git-ifies the chrome. New/changed surfaces, all 🟡 until their phase (R2–R5) lands: **(1) LINES, not branches** — the left `BRANCHES` panel + the top-bar branch switcher are removed; a `LINES` overview (lanes derived from `branchId`) replaces them, clicking a lane *focuses* it (never switches HEAD); the per-node branch chip is replaced by a **per-type state badge**; merge/push become **action nodes**, not verbs; "branch" → "line" everywhere. **(2) The hero chat** — a first-class, beautiful chat panel alongside the canvas (two views of the same node substrate); each turn becomes a node, the composer picks an agentic mode (Ask/Plan/Work), sending from a selected older node forks a new line. **(3) Family-driven Node-Details** — agentic nodes show {instruction, agent config, what-it-did, Thread}; action nodes show {recorded shell Output, Result}; all show {Codebase, Ancestry, Tree, + New node from here}. **(4) Per-type state badges** (agentic: thinking/awaiting-input/working/complete/require-review/require-info; action: pending/running/pass/fail) via the additive `NodeView.presentation_status` — forward-mapped (None) until the R3 producer lands. **(5) Open-in-editor** — "Open codebase" hands off to VS Code/Cursor (a `open_in_editor` Tauri command + a Settings editor preference); in-app Monaco stays read-only diff. **(6) The Orchestration MCP** — a write-capable surface your external agent drives (a one-checkbox "allow external orchestrator" gate). The honesty matrix (§13) and the §16 ritual govern the 🟡→🟢 flips. **The generic-CLI-as-model provider is deprecated** (it's the source of the "Test connection" errors) — Settings will route real agents to the Orchestration MCP and models to local-HTTP/cloud-BYOK.
>
> **R1 — Connectivity unblock (🟢 BUILT 2026-06-21).** The deprecation above is now shipped: the fictional `copilot-cli` default is removed from the router/registry/UI defaults (the top-bar model list and the Settings command prefill), the local OpenAI-compatible HTTP endpoint is the visible default route, and a CLI agent driven as a model now **fails loud with actionable guidance** ("this is an agentic CLI, not a model endpoint — connect it via the Orchestration MCP, or point Spork at a local HTTP endpoint") instead of a cryptic `exited with status 1`. The frozen `Transport`/`ProviderAdapter`/`CLI_PROVIDER_KEY` seams are untouched (a *conforming* JSONL program still resolves); only defaults + the failure message changed. The Settings CLI section carries an inline note steering real coding CLIs to the Orchestration MCP (R5).
>
> **R2 — View fields + de-git-ified UI + hero chat + open-in-editor (🟢 BUILT 2026-06-22).** Surfaces (1)–(5) above are now built. **Backend (R2a):** additive `Option` `NodeView` fields `presentationStatus`/`lineLabel`/`forkedFrom` (`presentationStatus` via a kind-gated `agent-*` payload read — `None` until the R3 producer; the others lineage-derived); `GRAPH_VIEW_SCHEMA_VERSION` → 2 (no migration). **(1) LINES, not branches:** the `BRANCHES` navigator → a `LINES` overview (lanes derived from `branchId`, rollup-state dots, fork ↳ marks, click to *focus* a lane — never switch HEAD); the TopBar branch switcher → an informational **line breadcrumb**; the NodeCard `branchId` chip → the **per-type state badge**; canvas **swimlane bands** (one per line, drawn behind nodes via `ViewportPortal`, with gutter labels + lane-focus dimming); merge reframed as "merge into another line" (line-labelled destinations); `NewBranch`/`BranchMenu`/the `newBranch` action removed; the node-verb toolbar → a **"+ New node from here ▾"** menu (Agentic: Ask/Plan/Explore/Work · Action: Run tests/Stress/Sanity/Commit/Push); StatusStrip "branch" → "line". **(2) The hero chat** (a canvas ⇄ chat TopBar toggle): the selected line rendered as a thread where every message *is* a node (kind/state/model/cost + "open as node"), a left spine echoing lineage, and a composer that picks an agentic mode (color-themed send) and creates a node — sending from a non-tip node starts a new line. **(3) Family-driven Node-Details:** agentic/context nodes lead with **Thread**, observing action/check nodes lead with **Result** (the recorded outcome — detailed shell `Output` honestly forward-mapped to R3/R4), all carry Changes + Info; "Branch" → "Line". **(5) Open-in-editor:** an `open_in_editor` Tauri command (explicit launcher → PATH probe → OS opener) + a Settings → Editor preference + an "Open codebase" affordance in Node-Details. **(4) state badges** ship forward-mapped (🟡, the lifecycle fallback) until the R3 producer emits `presentationStatus`. **(6) Orchestration MCP** stays 🟡 (R5). Green: 1163+ workspace · 96 app (Vitest) · 12 Tauri.

**The 14 BUILT commands** (the entire live action surface), all verified present in `crates/spork-ipc/src/command.rs` with live dispatch handlers in `crates/spork-daemon/src/dispatch.rs`: `NODE_CREATE`, `NODE_RESTORE`, `BRANCH_FORK`, `REF_CREATE`, `REF_MOVE`, `OP_UNDO`, `OP_REDO`, `GC_RUN`, `NODE_DIFF`, `BLOB_READ`, `NODE_RUN_CHECK`, `BRANCH_MERGE`, `GIT_EXPORT`, `GIT_PUSH` — plus the `graph_view` read and the `oplog-event` stream. *(Post-MVP these grew by `NODE_AGENT_RUN`/`NODE_AGENT_EDIT`/`PROJECT_IMPORT` + the P7 reads; the realignment appends `ORCHESTRATE_INVOKE` + an action-node command — all append-only.)*
