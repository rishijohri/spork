...
## P7.5 — Local Trusted-Agent MVP (MVP Readiness)
**Goal:** make Spork usable as a **minimum viable product before** the additive P8/P9 features. The contract-first spine froze every seam and proved it headless, but no phase owned the end-to-end user journey — so by P7 a user who opens a real repo gets a permanently empty canvas (no IPC/UI/daemon path mints the first node). **"1145+ tests / all core built" measured contract coverage, not journey reachability.** P7.5 is wiring-only over shipped F4 seams (additive `Command`/`AgentRunIntent` variants + Tauri commands + UI), editing no frozen contract. **Depends on:** P5, F4. **Design:** §17 (Phase 1 — Local MVP), §6.6, §8.2, §9.2, §11.1, §13.4, §15.1.

> **Full task-by-task detail (backend AND UI, with design refs + test plan) lives in [MVP_PLAN.md](MVP_PLAN.md). This list mirrors its §5 workstreams; that file is authoritative.**

**Build (Slice A — "usable today") ✅ done:**
- [x] **W1 Onboarding/Import** — additive `Command::ProjectImport` (append-only); daemon `cmd_project_import` (`spork-daemon/src/import.rs`) calls the existing `capture_working_tree` → root `snapshot` node (`origin=import`, best-effort git metadata) + branch/HEAD refs (`set_head_ref` create-or-move; `promote_ref` reused); refuses double-import; UI empty-state real `open_project → PROJECT_IMPORT` flow
- [x] **W2 Root at real project dir** — `DaemonBuilder::for_project` roots the daemon at the user's repo (workdir = project dir, state under hidden `.spork/`) via a new `with_workdir` + `STATE_SUBDIR`; ignore profile derives default+`.spork/` (additive, frozen default profile untouched); Tauri `open_project` uses it
- [x] **W3 Provider config from the app** — `AgentConfig` serde + `with_endpoint`/`with_cli_agent` builders + `Daemon::set_agent_config` (config behind its own `Mutex`); `set_agent_config` Tauri command + `DaemonRequest::SetAgentConfig` + `<project>/.spork/agent_config.json` persistence (load at open / save on set); Settings provider form (local endpoint / CLI + Test connection); top-bar model selector gated to backed providers (`availableModels`)
- [x] **W5a Transcript read fix** — agent run stores the `CanonicalTranscript` via `put_conversation` + binds `conversation_ref` on the node payload, so `get_node_transcript` (History MCP) returns it (was `None`)

**Build (Slice B — "agent does work") ✅ done:**
- [x] **W4 Trusted edit loop** — `AgentRunIntent::Change` + `Command::NodeAgentEdit` (append-only); daemon builds a `WorktreeCowBackend` over a 2nd handle to the CAS (`spork-daemon/src/edit.rs`); a bounded tool-loop (`read_file`/`write_file`/`list_dir`/`run_command`/`apply_patch`, **20 iter / $5 cap**) runs `run_turn` against a **CoW copy** (real checkout never touched) → `capture_path` → Edit node owning the new snapshot → §6.6 fork-on-divergence (tip continues / non-tip auto-forks `agent/<id>`) → auto-Sanity; "Make a change" intent in the Ask modal dispatches it; the Edit node renders (op-log + `attachEditNode`)
- [x] **W5b Conversation surface** — Node-Details **Conversation tab**: the node's stored transcript via the History MCP `get_node_transcript` (W5a populated `conversation_ref`) + a threaded read-only follow-up (`NodeAgentRun`). *(RunRail stdout tab stays honestly disabled — no streaming producer yet; live token streaming is deferred, post-MVP.)*

**Definition of Done (user terms):**
- [x] open a real repo → root snapshot node renders (empty-canvas dead-end gone); reopen rebuilds it *(Slice A)*
- [x] configure `copilot-cli`/a local endpoint in the app → a read-only ask returns a real answer; choice persists *(Slice A)*
- [x] ask the agent to make a change → an Edit node owns a new snapshot with a real diff + transcript + auto-Sanity; non-tip → auto-forks; real workdir untouched *(Slice B — W4)*
- [x] a node's transcript is retrievable via the History MCP `get_node_transcript` *(W5a)* and renders in the Node-Details Conversation tab *(W5b)*
- [x] full green bar (`cargo test/clippy/fmt`, app `tsc`/vitest/build) for both slices; UI_UX honesty matrix re-tagged 🟡→🟢 for the unlocked onboarding/provider/edit/conversation surfaces

**Out of scope (documented, additive — not stubs):** untrusted-executor isolation (the real P8), cloud **TLS** transport, live token **streaming** over `CHAT_TOKENS`.

**Status:** **Slices A + B DONE & verified** ✅ — W1·W2·W3·W4·W5a·W5b all landed behind frozen seams (additive `Command`/`AgentRunIntent` variants + new daemon modules + Tauri command + UI; no frozen contract edited). Core workspace **1163 tests** green (35 crates, +18 over P7) + clippy `-D warnings` + rustfmt clean; Tauri app **10** tests + `cargo check`/clippy/fmt green; app **90** Vitest + `tsc` + `vite build` green; no stub macros. **Spork is now an MVP**: open a real repo → see code → configure a local/CLI agent → ask it to make a change → get an Edit node (diff + transcript + auto-Sanity) that auto-forks on divergence → branch/merge/restore → reopen intact. Full detail: [MVP_PLAN.md](MVP_PLAN.md).

**Post-launch fixes (after first real `tauri:dev` run — found via a 9-agent adversarial UI audit):**
- [x] **Native folder picker** — added the Tauri v2 `dialog` plugin (JS + Rust + `dialog:default` capability + `.plugin(tauri_plugin_dialog::init())`); a "Browse…" button opens the OS folder dialog (the empty state had only a type-a-path field). `pickProjectDir()` is mock-safe.
- [x] **Settings popover off-screen (double-absolute)** — `.spork-popover` was `position:absolute` inside an already-absolute `right:0` wrapper → overflowed off the right edge; changed to `position:relative` + viewport max-w/max-h clamps.
- [x] **Reopen-a-project blocker** — `project.import` is now **idempotent** (a reopen returns the existing root, no error); the onboarding flow (`openAndImport`) always refetches `graph_view`, so a reopened project renders instead of showing a red "Open failed" with a stuck empty canvas.
- [x] **Project persistence + auto-reopen** — the last opened project is remembered (localStorage) and auto-reopened on launch (Shell, desktop-only); the canvas shows an "Opening project…" spinner (gated on `projectOpening`) instead of flashing the picker.
- [x] **Context-menu off-screen** — right-click menu now viewport-clamps (JS `getBoundingClientRect` clamp + CSS `max-height`/`overflow-y`), same class as the popover bug.
- [x] **Inline onboarding errors** — open/import failures render inline in the empty state (`role="alert"`), not only in the collapsible activity rail.
- [ ] **Deferred — large-repo import freezes the UI** (audit major): `capture_working_tree` runs synchronously on the daemon owner thread, so importing a big repo blocks the IPC loop with only a spinner. Mitigated for now by the honest "Opening project…" spinner; the real fix (off-owner-thread capture + a progress stream + optional cancel via a CancellationToken threaded into `put_tree`) is a P8-sized follow-up.

---

## Realignment (R0–R6) — Timeline-First · Node-Centric · Platform-Inverted
**Goal:** Re-center Spork on its thesis after the MVP UI drifted git-ward. Full detail in **[REALIGNMENT_PLAN.md](REALIGNMENT_PLAN.md)** (authoritative for this track). **Additive over the frozen F0–F4/P5–P7 contracts** (no domino — a §7 C2 risk register, adversarially audited). **Depends on:** P7.5. **Design:** REALIGNMENT_PLAN + DESIGN.md realignment note.

- [x] **R0 — Docs realignment (no code)** ✅ — REALIGNMENT_PLAN.md written; IDEA/DESIGN/UI_UX/CLAUDE/IMPLEMENTATION_PLAN updated (nodes-spine, branches-emergent, agentic+action node-type families + per-type states, the Orchestration MCP, the connectivity decision, hero chat, open-in-editor). *Review gate — awaiting go-ahead to build R1+.*
- [x] **R1 — Connectivity unblock** ✅ — unwired the fictional `copilot-cli` from the router binding default (`CliAdapter::DEFAULT_MODEL` is now the neutral `"cli-agent"`), the registry capability seed, the top-bar `MODELS` list, the Settings command prefill, and the mock fallback label; fixed the `agent.rs`/`lib.rs` doc examples; added `CLI_AS_MODEL_GUIDANCE` + `map_agent_error_for` so the daemon's CLI-as-model failure path **fails loud with guidance** (run + edit loops) instead of `exited with status 1`; the local OpenAI-compatible HTTP endpoint is the visible default route; Settings CLI section carries an inline note steering real coding CLIs to the Orchestration MCP (R5). **Soft-deprecation (C2):** the frozen `Transport`/`ProviderAdapter`/`CLI_PROVIDER_KEY` seams stay append-only (a *conforming* JSONL program still resolves — DoD tests green); only defaults + the failure message changed. *(No new contracts.)*
- [x] **R2 — Additive view fields + UI de-git-ification + hero chat + open-in-editor** ✅ — **R2a** additive `Option` `NodeView` fields (`presentation_status` kind-gated `agent-*` payload read — `None` until the R3 producer; `line_label`/`forked_from` lineage-derived in `build_graph_view`); `GRAPH_VIEW_SCHEMA_VERSION` → 2 (no migration); TS `NodeView` mirrors field-for-field. **R2b de-git-ification:** `state/lines.ts` (`deriveLines`); Navigator `BRANCHES`→`LINES` (rollup dots, fork marks, focus-a-lane); TopBar branch switcher → informational **line breadcrumb** + canvas⇄chat toggle; NodeCard chip → **state badge** (`statusBadge`, presentation-aware w/ lifecycle fallback); Canvas **swimlanes** (`laneLayout` + `ViewportPortal` bands + lane-focus dim); toolbar → **"+ New node from here ▾"** (Agentic/Action) + Restore + Merge-into-line; removed `NewBranchModal`/`BranchMenu`/`newBranch`; reframed Merge (line destinations), StatusStrip/CommandPalette/ContextMenu line-language; store dropped `newBranch`/`branch` variants, added `focusedLane`/`focusLane`/`centerView`. **R2c hero chat** (`HeroChat.tsx` + `ipc/transcript.ts`): the line as a thread where every message *is* a node, a lineage spine, a mode-themed composer that creates nodes (non-tip → new line). **R2d family-driven Node-Details:** Thread (agentic) / Result (observing, shell Output 🟡→R3/R4) / Changes / Info; Branch→Line. **R2e open-in-editor:** `open_in_editor` Tauri command (launcher→PATH probe→OS opener) + Settings Editor pref + Node-Details affordance. *Forward-mapped (honest):* the `presentation_status` **producer** (R3 turns the badge live), the detailed **recorded shell Output** (R3/R4). Green: **1163 ws + 96 app (Vitest) + 10 Tauri**, clippy `-D warnings` + rustfmt clean, tsc + vite build clean, no stub macros.
- [ ] **R3 — New node-type descriptors + dispatch + state producer** — register the agentic (`agent-plan/ask/explore/work`) + action (`action-run-tests/stress/sanity`) descriptors; new daemon dispatch that stamps per-kind `kind` + rich payload (config in payload, mapped onto the existing `AgentRunIntent`); the agent-loop producer that emits `presentation_status` (badge 🟡→🟢).
- [ ] **R4 — git-push/commit as real nodes** — a new append-only node-producing command + `GitPushRunner`/`GitCommitRunner` emitting `NodeCreated` + recording output on a `ResultEnvelope` (never edit the frozen `GIT_PUSH`/`GIT_EXPORT`).
- [ ] **R5 — Orchestration MCP ("Spork Node skill")** — new `spork-orchestrate` crate + `Command::OrchestrateInvoke` (+ optional `NodeSetPresentation`) + stdio shim binary + capability-gating DoD tests + the `spork-node` SKILL/registration doc; UI: "allow external orchestrator" checkbox.
- [ ] **R6 — TLS BYOK transport** — a new `TlsHttpTransport` impl behind the frozen `Transport` seam + a TLS dep + a `vaultRef` on `AgentConfig` (capable cloud models for the Spork-drives-model arrow).

**Open decisions:** OD-1 one axis (`kind`→`intent`, no parallel `mode`) *(adopted)*; **OD-2 `require_review → Blocked` — ✅ USER-CONFIRMED 2026-06-22** (dependents stall until a human acts; the safe, engine-honest choice — locked for R3); OD-3 new `agent-work` descriptor (not an `EditPayload` migration) *(adopted)*; **OD-4 one global "allow external orchestrator" gate for now — ✅ USER-CONFIRMED 2026-06-22** (per-client scoping via the §15.1 grammar is later — locked for R5); OD-5 ship R1+R2 first *(done)*.

**Out of scope (documented, additive):** embedding a real agent CLI as a delegate (its own `stream-json`) — future P8 node-type + executor; per-orchestrator capability scoping (DESIGN §15.1 grammar, D-8).

**Status:** **R0 + R1 + R2 DONE & verified** ✅ — green bar after R2: **1163 workspace + 96 app (Vitest) + 10 Tauri tests**, clippy `-D warnings` + rustfmt clean (workspace + Tauri), tsc + vite build clean, no stub macros. R1 soft-deprecated the generic-CLI-as-model route; R2 added the additive view fields + the full timeline-first UI reframe (LINES/swimlanes/state-badges, the hero chat, family-driven Node-Details, open-in-editor) — all additive, no frozen contract edited. **Next: R3** (agentic + action node-type descriptors + new daemon dispatch + the `presentation_status` producer that turns the state badge live). **⚠️ R3/R5 want explicit confirmation first:** OD-2 (`require_review → Blocked`) and OD-4 (one global "allow external orchestrator" gate).
